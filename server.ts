...
async function startServer() {
  ...
  app.post("/api/grounded-chat", async (req, res) => {
    try {
  ...
  app.post("/api/schedule", async (req, res) => {
    // Logic for scheduling segments
    res.json({ success: true, message: "Schedule created (placeholder)" });
  });

  app.post("/api/playlist", async (req, res) => {
    // Logic for generating playlist
    res.json({ success: true, message: "Playlist created (placeholder)" });
  });
      ...
      try {
        const session = await ai.live.connect({
          ...
          config: {
            ...
            systemInstruction: "You are a helpful romantic assistant for the Romance Radio Studio.",
          },
        });
